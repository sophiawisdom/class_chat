...
def load_database():
    with open("wyr/questions.txt") as file:
        questions = file.read().split("\n")
    questions = [q for q in questions if (not q.startswith("#")) and q]
    types = {}
    m = 0
    n = [questions.index(i + ":") for i in categories]
    n.append(len(questions)-1) # for end point
    returndict = {}
    for place, value in enumerate(categories):
        returndict[value] = questions[n[place]+1:n[place+1]]
    return returndict
